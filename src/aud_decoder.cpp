...
void ima_adpcm_ws_decoder_t::decode(uint8_t *in, size_t size, int16_t *out)
{
	uint8_t *end = in + size;

	int16_t  step_index = _step_index;
	int32_t  predictor  = _predictor;
	...
	while (in != end)
	{
		...
		for (int n = 0; n != 2; ++n)
		{
			...
			predictor = std::min(std::max(predictor, -32768), 32767);

			*out++ = predictor;

			step_index = ima_index_table[code] + step_index;
			step_index = std::min(std::max(step_index, int16_t(0)), int16_t(88));
		}
	}
...
aud_decoder_t::aud_decoder_t(reader_t *r)
	: r(r)
{
	r->read_le16(&header.sample_rate);
	r->read_le32(&header.size);
	r->read_le32(&header.out_size);
	r->read_byte(&header.flags);
